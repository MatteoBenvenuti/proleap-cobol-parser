...
public class SM301MTest extends CobolTestBase {

	@Test
	public void test() throws Exception {
		final File inputFile = new File("src/test/resources/gov/nist/SM301M.CBL");
		final Program program = new CobolParserRunnerImpl().analyzeFile(inputFile, CobolSourceFormatEnum.FIXED);
		final CompilationUnit compilationUnit = program.getCompilationUnit("SM301M");
...

		{
...

			{
				...
				assertEquals("SM301M-CONTROL", paragraph.getParagraphName().getName());
...

			{
				...
				assertEquals("SM301M-COPY", paragraph.getParagraphName().getName());
